...
## 2026-05-07 SAM3-driven Inpaint360GS runner (raw SAM stage replaced)

- Added `tools/run_sam3_inpaint360gs_pipeline.py` to run an Inpaint360GS flow
  that replaces the original `raw_mask_sam.py` stage with
  `tools/generate_sam3_instance_masks.py` (SAM3 local checkpoint).
- New runner preserves existing downstream folder contracts and reuses current
  stage adapters:
  - SAM3 instance masks -> `data/inpaint360/<scene>/raw_sam`
  - segment reduction (`tools/reduce_sam_segments.py`)
  - run stages `3..9` via `tools/run_inpaint360gs_full.py --start-stage 3 --stop-after-stage 9`
  - optional virtual-depth fixes (`inpaint360_align_completed_depth.py`,
    `inpaint360_project_completed_to_hole_plane.py`)
  - resume stages `10..11` (`--start-stage 10`) with optional checkpoint videos.
- Purpose: reduce mask instability from legacy raw SAM stage while keeping
  compatibility with current object-removal/distillation/fusion/inpaint code.

## 2026-05-07 SAM3 direct-target workflow (separate folder/files)

- Added separate files under `tools/sam3_direct/`:
  - `run_sam3_direct_pipeline.py`
  - `README.md`
- This workflow bypasses legacy stages 2..6 and uses prompt-driven SAM3 masks
  directly:
  1) SAM3 masks on train views
  2) projection-prune Gaussians (`prune_3dgs_with_inpaint360gs_masks.py`)
  3) virtual renders from original + pruned models
  4) SAM3 masks on virtual renders -> `inpaint_2d_unseen_mask_virtual`
  5) existing LaMa + optional depth fixes
  6) existing stage10/11 fusion + inpaint + eval
- Kept this path isolated from baseline scripts to reduce regression risk.

## 2026-05-06 Inpaint360GS `bag`: virtual fusion + depth alignment verified

- Full one-scene driver + stage table + novelty notes + checkpoint videos during finetune:
  **`project_memory/INPAINT360GS_FULL_PIPELINE_RUNBOOK.md`**
