...
def removal_setup(model_path, iteration, gaussians, classifier, selected_obj_ids, removal_thresh):
    """
    ...
    with torch.no_grad():
        ...
        for i, obj_id in enumerate(selected_obj_ids):
            ...
            # Fall back to max-index matching if threshold gives no Gaussians
            if mask3d.sum() == 0:
                print(f"  [removal_setup] No Gaussians above threshold {removal_thresh} for obj {obj_id_int}, using max-index fallback")
                mask = (max_indices == obj_id).squeeze()
                mask3d = mask.squeeze()

            if mask3d.sum() == 0:
                print(f"  [removal_setup] Still no Gaussians for obj {obj_id_int}, skipping")
                continue
    ...
    if len(selected_obj_ids)>1: 
        ...
        with torch.no_grad():
            for obj_id in selected_obj_ids:
                obj_id_int = int(obj_id.item())
                if obj_id_int not in masks_per_obj:
                    continue
    ...
    if not masks_per_obj:
        print("  WARNING: No objects found for removal, using default radius")
        target_object_radius = 1.0
    elif len(selected_obj_ids) > 1 and 'total_radius' in dir():
        target_object_radius = total_radius
    else:
        first_key = next(iter(masks_per_obj))
        target_object_radius = masks_per_obj[first_key]["radius"]
...
def removal(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, opt : OptimizationParams, select_obj_id : int, removal_thresh : float, render_video : bool):
    ...
    gaussians, objects_model_dict, target_object_radius = removal_setup(dataset.model_path, scene.loaded_iter, gaussians, classifier, select_obj_id, removal_thresh)

    # scene_json_path = os.path.join(dataset.source_path, "associated_hqsam", "scene.json")
    update_config_radius(args.config_file, target_object_radius)
    ...
    if len(select_obj_id) > 1 and len(args.target_id) < len(select_obj_id):
        gaussians= combine_gaussian(dataset, str(scene.loaded_iter), gaussians, objects_model_dict, select_obj_id)
...
if __name__ == "__main__":
    ...
    for _obj_dir in ("associated_sam", "associated_hqsam"):
        scene_json_path = os.path.join(args.source_path, _obj_dir, "scene.json")
    ...
    removal(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, opt.extract(args), args.select_obj_id, args.removal_thresh, args.render_video)
