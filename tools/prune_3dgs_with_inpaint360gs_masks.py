...
def _erode_binary_mask(mask: np.ndarray, pixels: int) -> np.ndarray:
    """Shrink True regions (fewer Gaussians marked for removal at jittery SAM boundaries)."""
    if pixels <= 0:
        return mask
    try:
        import cv2
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Apply binary erosion to each SAM mask before projecting (reduces inconsistent "
        "boundary pruning when masks jitter across views).",
    )
...
def main():
    ...
    for image_name, mask_path in view_specs:
        ...
        mask = _erode_binary_mask(mask, args.mask_erode_pixels)
