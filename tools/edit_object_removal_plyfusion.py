...
#
# NOTE: Mirror of upstream_overrides/external Inpaint360GS fusion with depth grid fixes.
# If you patch fusion, sync this copy so accidental PYTHONPATH misuse does not resurrect the
# stale no-resize fusion path.
...
def _squeeze_depth_2d(depth_map: np.ndarray, view_name: str) -> np.ndarray:
    ...
    if z.ndim != 2:
        raise ValueError(
            f"{view_name}: expected depth map to be HxW after squeeze, "
            f"got shape {depth_map.shape} -> {z.shape}"
        )
    return z
...
def fusion(dataset_path, model_path, name, iteration, views):
    ...
    fused_mask_col_dep_ply_path = os.path.join(
        model_path, name, f"ours_object_removal/iteration_{iteration}", "fused_mask_col_dep_ply"
    )
    fused_hole_col_dep_ply_path = os.path.join(
        model_path, name, f"ours_object_removal/iteration_{iteration}", "fused_hole_col_dep_ply"
    )
    ...
    for _, view in enumerate(tqdm(virtual_poses_list, desc="Color-Depth-Fusion progress")):
        ...
        possible_paths = [
            os.path.join(dataset_path, "images_inpaint_unseen_virtual", view.image_name + ext) for ext in extensions
        ]
        ...
        depth_hole = _squeeze_depth_2d(
            np.load(os.path.join(depth_hole_path, view.image_name + ".npy")), view.image_name + " [hole]"
        )
        ...
        if depth_completed.shape != depth_hole.shape:
            ...
            depth_completed = cv2.resize(depth_completed, (dw, dh), interpolation=cv2.INTER_LINEAR)
            print(
        ...
        if colors_hole_img.shape[:2] != depth_hole.shape[:2]:
            dh2, dw2 = depth_hole.shape[:2]
            colors_hole_img = cv2.resize(colors_hole_img, (dw2, dh2), interpolation=cv2.INTER_LINEAR)
